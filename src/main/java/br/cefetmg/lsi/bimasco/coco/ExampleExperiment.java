...
public class ExampleExperiment {
	 ...
	 */
	public static BenchmarkProblem PROBLEM;
	...
	public static void exampleExperiment(String suiteName, String observerName, Random randomGenerator) {
		try {
			...
			CoCOBenchmark coCOBenchmark = new CoCOBenchmark(suite, observer);

			/* Initialize timing */
			Timing timing = new Timing();
			
			/* Iterate over all problems in the suite */
			while ((PROBLEM = coCOBenchmark.getNextProblem()) != null) {
	...
	public static void myRandomSearch(Function f, 
		...
		for (int i = 0; i < maxBudget; i++) {
			...
			IntStream.range(1, 500).parallel().forEach((it) -> {
				f.evaluate(x);
			});
			y = f.evaluate(x);
		}
